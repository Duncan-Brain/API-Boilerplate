...
kubectl --kubeconfig=$TRAVIS_BUILD_DIR/config-travisci-sa apply -f k8s
kubectl --kubeconfig=$TRAVIS_BUILD_DIR/config-travisci-sa set image deployments/frontend-deployment frontend=duncanbrain/boiler-api-frontend:$SHA
kubectl --kubeconfig=$TRAVIS_BUILD_DIR/config-travisci-sa set image deployments/api-deployment api=duncanbrain/boiler-api-api:$SHA
